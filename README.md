...
- **导出与离线查看**：将当前城市或整个项目导出为图片包、Word 或 Excel。Word、Excel 先放所有天的整体行程图，再按日期放当天行程图，均带高德底图与已保存路线；下载图片时，将总览和每天各自的 PNG 打包为 ZIP。每段仅展示实际确认的起点、终点和交通方式。地图生成失败时可重试，不替换成无底图轨迹。
...
管理员可统一设置 DeepSeek 模型 ID，AI 助手和单条补充均使用后台配置。浏览器任务列表最多保留最近 5 条；后端在项目 AI 服务初始化及运行期间每分钟清理创建超过 3 天的已完成、失败和已导入任务，排队和生成中的任务保留，已导入的行程不受影响。

后台保存的模型 ID 和密钥位于数据目录的 `api-settings.json`，权限为 `0600`，优先于对应环境变量。配置保存后新请求生效，重启后继续使用；模型 ID 可查看和修改，密钥仅显示配置状态，不回显。没有配置 AI 或高德时，仍可手动维护旅行计划。
